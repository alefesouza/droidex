...
public class ActivityPokemonFilter extends Activity {
    ...
    public void finish() {
        ...
        if (this.generation.equals(generation) &&
                ...
                this.isBaby == isBaby &&
                this.hasGenderDiff == hasGenderDiff) setResult(RESULT_CANCELED);
        else {
    ...
    private void restorePreviousFilter() {
        if (!this.generation.isEmpty()) {
            ...
            for (String aGen : gen) listChkGeneration.get(Integer.valueOf(aGen) - 1).setChecked(true);
        }

        if (!this.type.isEmpty()) {
            String[] type = this.type.split(", ");
            for (String aType : type) listChkType.get(Integer.valueOf(aType) - 1).setChecked(true);
        }

        if (!this.color.isEmpty()) {
            String[] color = this.color.split(", ");
            for (String aColor : color) listChkColor.get(Integer.valueOf(aColor) - 1).setChecked(true);
        }
    ...
    private void setCLickListener() {
        ...
        txtClearFilter.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {clearFilter();}
        });
    }
